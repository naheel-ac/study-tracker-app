...
void main() {
  ...
  setUp((){
    ...
    when(() => mockSignUp.call(any(), any())).thenAnswer((_)async => null);
    when(() => mockSignOut.call()).thenAnswer((_)async => {});
  });
  testWidgets('Counter increments smoke test', (WidgetTester tester) async {
    // Build our app and trigger a frame.
    await tester.pumpWidget(BlocProvider(create: (_) => AuthBloc(getCurrentUserUseCase: mockGetCurrentUser, signInUseCase: mockSignIn, signUpUseCase: mockSignUp, signOutUseCase: mockSignOut)..add(AuthCheckRequest()), child: const MaterialApp(home: SignInScreen(),),));
    await tester.pump();
